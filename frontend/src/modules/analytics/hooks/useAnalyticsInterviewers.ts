...
export const useAnalyticsInterviewers = (
  ...
  options: { interviewerIds?: string[]; roles?: InterviewerSeniority[]; groupBy?: TimelineGrouping }
): HookState => {
  ...
  }, [period, options.groupBy, options.interviewerIds?.join(','), options.roles?.join(',')]);
