...
export const AnalyticsScreen = () => {
  ...
  const [interviewerGraphPeriod, setInterviewerGraphPeriod] = useState<InterviewerPeriod>('last_month');
  const [interviewerGraphGrouping, setInterviewerGraphGrouping] = useState<TimelineGrouping>('month');
  const initialGraphRange = deriveInterviewerRange('last_month');
  ...
  const interviewerGraphState = useAnalyticsInterviewers(interviewerGraphPeriod, {
    interviewerIds: selectedInterviewers.length ? selectedInterviewers : undefined,
    roles: selectedRoles.length ? selectedRoles : undefined,
  ...
  useEffect(() => {
    const range = deriveInterviewerRange(interviewerGraphPeriod);
    setInterviewerGraphFrom((current) => (current === range.from ? current : range.from));
    setInterviewerGraphTo((current) => (current === range.to ? current : range.to));
  }, [interviewerGraphPeriod]);
  ...
  return (
    <div className={styles.screen}>
      ...
      <InterviewerGraphSection
        period={interviewerGraphPeriod}
        onPeriodChange={setInterviewerGraphPeriod}
