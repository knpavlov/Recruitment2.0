...
export const InterviewerGraphSection = ({
  ...
  const aggregates = useMemo(() => (data ? aggregateInterviewerGraphBuckets(data) : []), [data]);
  ...
  const points = useMemo<ChartPoint[]>(() => {
    ...
    return aggregates.map((bucket) => ({
      ...
      label: formatBucketLabel(bucket.bucket, data.groupBy),
      interviews: bucket.interviews,
      hireShare: bucket.hireShare,
      caseScore: bucket.caseScore,
      fitScore: bucket.fitScore
    }));
  }, [aggregates, data]);
