...
export const InterviewerGraphSection = ({
  ...
  const rangeLabel = useMemo(() => {
    ...
    if (hasFrom && hasTo) {
      return `${formatter.format(new Date(fromValue))} – ${formatter.format(new Date(toValue))}`;
    }
    if (hasFrom) {
      return `from ${formatter.format(new Date(fromValue))}`;
    }
    if (hasTo) {
      return `until ${formatter.format(new Date(toValue))}`;
    }
    ...
    <section className={styles.sectionCard}>
      <header className={styles.sectionHeader}>
        <div>
          ...
          <p className={styles.metricDetails}>Average metrics per interviewer for {rangeLabel}</p>
        </div>
      </header>
