...
export const InterviewerGraphSection = ({
  ...
  const roleSet = useMemo(
    () => (selectedRoles.length ? new Set(selectedRoles) : null),
    [selectedRoles]
  );
  ...
  const points = useMemo<ChartPoint[]>(() => {
    ...
    const filteredBuckets = data.buckets.filter((bucket) => {
      const matchesInterviewer =
        !interviewerSet || interviewerSet.has(bucket.interviewerId.toLowerCase());
      const matchesRole = !roleSet || (bucket.interviewerRole && roleSet.has(bucket.interviewerRole));
      return matchesInterviewer && matchesRole;
    });
