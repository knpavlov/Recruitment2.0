...
const PERIOD_LABELS: Record<SummaryPeriod, string> = {
  rolling_3: 'Скользящее среднее за 3 месяца',
  fytd: 'С начала финансового года',
  rolling_12: 'Скользящее среднее за 12 месяцев'
};
...
const formatDetails = (numerator: number, denominator: number) => {
  ...
  return `${numerator} из ${denominator}`;
};
  ...
  return (
    <section className={styles.sectionCard}>
      <header className={styles.sectionHeader}>
        <div>
          <h2 className={styles.sectionTitle}>Сводка по воронке</h2>
      ...
      {loading ? <div className={styles.loadingLabel}>Загружаем метрики…</div> : null}

      {!loading && !error && metrics ? (
        <div className={styles.cardsGrid}>
          <article className={styles.metricCard}>
            <h3 className={styles.metricTitle}>Доля женщин среди кандидатов</h3>
          ...
          <article className={styles.metricCard}>
            <h3 className={styles.metricTitle}>Доля принятых офферов</h3>
