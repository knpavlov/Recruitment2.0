...
export type SeriesValueType = 'count' | 'percentage' | 'score';
...
interface TimelineChartProps<T extends { bucket: string }> {
  points: T[];
...
export const TimelineChart = <T extends { bucket: string }>({
  points,
  series,
  ariaLabel
      ...
      ...points.map((point) =>
        Math.max(
          ...countsSeries.map((item) => {
            const value = point[item.key as keyof T];
  ...
  const countPaths = countsSeries.map((item) => {
    const values = points.map((point) => {
      const raw = point[item.key as keyof T];
  ...
  return (
    <div className={styles.timelineWrapper}>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} width="100%" role="img" aria-label={ariaLabel}>
        ...
        {/* Right Y labels for percentages/scores */}
        {[0, 0.5, 1].map((value) => (
          <text
            key={`y-right-${value}`}
