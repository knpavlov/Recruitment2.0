...
export const TimelineChart = ({ points, series }: TimelineChartProps) => {
  ...
  return (
    <div className={styles.timelineWrapper}>
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} width="100%" role="img" aria-label="Динамика ключевых показателей">
        <defs>
        ...
        {/* Подписи по оси Y для количественных показателей */}
        {[0, 0.5, 1].map((value) => (
          <text
            key={`y-left-${value}`}
            x={PADDING_X - 16}
        ...
        {/* Подписи по оси Y для процентов/баллов */}
        {[0, 0.5, 1].map((value) => (
          <text
            key={`y-right-${value}`}
