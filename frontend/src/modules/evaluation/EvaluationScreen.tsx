...
import { EvaluationConfig, OfferRecommendation } from '../../shared/types/evaluation';
...
const OFFER_ORDER: OfferRecommendation[] = ['yes-priority', 'yes', 'hold', 'no'];

// Рассчитываем процентное распределение рекомендаций оффера для отображения в таблице
const formatOfferBreakdown = (forms: EvaluationConfig['forms']): string => {
  const recommendations = forms.filter(
    (form) => form.submitted && form.offerRecommendation && OFFER_ORDER.includes(form.offerRecommendation)
  );
  if (!recommendations.length) {
    return '—';
  }

  const counts = OFFER_ORDER.map(
    (option) => recommendations.filter((form) => form.offerRecommendation === option).length
  );
  ...
  if (roundedTotal !== 100) {
    ...
    let indexToAdjust = counts.findIndex((count) => count === maxCount && count > 0);
    if (indexToAdjust === -1) {
      indexToAdjust = 0;
    }
    rounded[indexToAdjust] = Math.max(0, Math.min(100, rounded[indexToAdjust] + delta));
  }

  return rounded.map((value) => `${value}%`).join(' / ');
};
...
export const EvaluationScreen = () => {
  ...
  const tableRows = useMemo<EvaluationTableRow[]>(() => {
    return list.map((evaluation) => {
      ...
      const breakdown = formatOfferBreakdown(evaluation.forms);
