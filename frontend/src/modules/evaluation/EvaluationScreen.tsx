...
export const EvaluationScreen = () => {
  ...
  const tableRows = useMemo<EvaluationTableRow[]>(() => {
    return list.map((evaluation) => {
      ...
      const storedDecisionRaw = snapshot ? snapshot.decision ?? null : evaluation.decision ?? null;
      const storedDecision = storedDecisionRaw === 'accepted-offer' ? 'offer' : storedDecisionRaw;
      ...
      const storedStatusRaw = snapshot
        ? snapshot.offerDecisionStatus ?? null
        : evaluation.offerDecisionStatus ?? null;
      const storedStatus: OfferDecisionStatus | null =
        storedDecisionRaw === 'accepted-offer' && (!storedStatusRaw || storedStatusRaw === 'pending')
          ? 'accepted'
          : storedStatusRaw;
      ...
      const effectiveStatus: OfferDecisionStatus = overrideStatus ?? storedStatus ?? 'pending';
