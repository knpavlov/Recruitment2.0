...
const formatOfferRecommendation = (value: OfferRecommendation | undefined) =>
  value ? OFFER_RECOMMENDATION_LABELS[value] : 'Not provided';
                ...
                return (
                  <li key={form.slotId} className={styles.statusRow}>
                    <div>
                      ...
                      {(form.interestLevel || form.issuesToTest) && (
                        <p className={styles.statusNotes}>
                          {form.interestLevel && (
                            <>
                              <strong>Interest:</strong> {form.interestLevel}
                            </>
                          )}
                          {form.interestLevel && form.issuesToTest && <br />}
                          {form.issuesToTest && (
                            <>
                              <strong>Next rounds:</strong> {form.issuesToTest}
                            </>
                          )}
                        </p>
                      )}
                      {(form.offerRecommendation || form.offerRecommendationNotes) && (
                        <p className={styles.statusNotes}>
                          <strong>Offer decision:</strong> {formatOfferRecommendation(form.offerRecommendation)}
                          {form.offerRecommendationNotes ? ` — ${form.offerRecommendationNotes}` : ''}
                        </p>
                      )}
                      {form.notes && (
                        <p className={styles.statusNotes}>
