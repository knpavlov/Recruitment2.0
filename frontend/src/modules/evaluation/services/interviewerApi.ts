...
const normalizeCriteriaList = (value: unknown): EvaluationCriterionScore[] => {
  ...
  const appendEntry = (entry: unknown, fallbackId?: string) => {
    if (entry && typeof entry === 'object') {
      ...
      const snakeId = !explicitId ? normalizeString(payload.criterion_id)?.trim() : undefined;
      const camelAlt = !explicitId ? normalizeString(payload.criteriaId)?.trim() : undefined;
      const legacyId = !explicitId ? normalizeString(payload.id)?.trim() : undefined;
      const legacyKey = !explicitId ? normalizeString(payload.key)?.trim() : undefined;
      const criterionId =
        explicitId || snakeId || camelAlt || legacyId || legacyKey || (fallbackId ?? '');
      if (!criterionId) {
        return;
      }
      const rawScore =
        payload.score ?? payload.value ?? payload.rating ?? payload.result ?? payload.points ?? entry;
