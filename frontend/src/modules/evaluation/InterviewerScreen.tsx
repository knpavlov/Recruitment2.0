...
const computeAverageScore = (
  ...
  const allowed = criteria.length ? new Set(criteria.map((criterion) => criterion.id)) : null;
  const numericScores: number[] = [];
  for (const [criterionId, rawValue] of Object.entries(values)) {
    if (allowed && !allowed.has(criterionId)) {
      continue;
    }
  ...
  const sum = numericScores.reduce((accumulator, value) => accumulator + value, 0);
  return Math.round((sum / numericScores.length) * 10) / 10;
};

const reorderCaseCriteria = (criteria: CriterionDefinition[]): CriterionDefinition[] => {
  if (!criteria.length) {
    return [];
  }
  const used = new Set<string>();
  const prioritized: CriterionDefinition[] = [];
  for (const title of CASE_CRITERIA_ORDER) {
    const match = criteria.find((criterion) => criterion.title === title && !used.has(criterion.id));
    if (match) {
      prioritized.push(match);
      used.add(match.id);
    }
  }
  const remaining = criteria.filter((criterion) => !used.has(criterion.id));
  return [...prioritized, ...remaining];
};

const formatScoreLabel = (value: number | undefined): string => {
...
const AverageScoreCard = ({ title, value }: { title: string; value: number | undefined }) => (
  <div className={`${styles.criterionCard} ${styles.averageCard}`}>
    <span className={styles.averageTitle}>{title}</span>
    <span className={styles.averageValue}>{formatScoreLabel(value)}</span>
  </div>
);
...
export const InterviewerScreen = () => {
  ...
  const buildCriteriaPayload = (
    ...
    return Object.entries(values)
      .map(([criterionId, scoreValue]) => {
        if (!criterionId || (allowed.size > 0 && !allowed.has(criterionId))) {
          return null;
        }
  ...
  const persistForm = async ({ submitted }: { submitted: boolean }) => {
    ...
    setBanner(null);
    try {
      const fitCriteriaDefinitions = selectedAssignment.fitQuestion?.criteria ?? [];
      const caseCriteriaDefinitions = selectedAssignment.caseFolder?.evaluationCriteria ?? [];
      const computedFitScore = computeAverageScore(formState.fitCriteria, fitCriteriaDefinitions);
      const computedCaseScore = computeAverageScore(formState.caseCriteria, caseCriteriaDefinitions);
      const fitScoreToSubmit =
        computedFitScore ?? (selectedAssignment.form?.fitScore ?? undefined);
      const caseScoreToSubmit =
        computedCaseScore ?? (selectedAssignment.form?.caseScore ?? undefined);

      await interviewerApi.submitForm(selectedAssignment.evaluationId, selectedAssignment.slotId, {
        email: session.email,
        submitted,
        fitScore: fitScoreToSubmit,
        caseScore: caseScoreToSubmit,
        ...
        fitCriteria: buildCriteriaPayload(formState.fitCriteria, fitCriteriaDefinitions),
        caseCriteria: buildCriteriaPayload(formState.caseCriteria, caseCriteriaDefinitions)
      });
  ...
  const renderDetail = () => {
    ...
    const caseCriteria: CriterionDefinition[] = reorderCaseCriteria(
      selectedAssignment.caseFolder?.evaluationCriteria ?? []
    );
    const behaviouralAverage =
      computeAverageScore(formState.fitCriteria, fitCriteria) ?? selectedAssignment.form?.fitScore;
    const caseAverage =
      computeAverageScore(formState.caseCriteria, caseCriteria) ?? selectedAssignment.form?.caseScore;
        ...
        <div className={styles.detailHeader}>
          <div>
            ...
            <div className={styles.detailMeta}>
              {candidate?.targetOffice && <span>Target office: {candidate.targetOffice}</span>}
              {candidate?.desiredPosition && <span>Target role: {candidate.desiredPosition}</span>}
            </div>
          </div>
              ...
              <section className={styles.formSection}>
                ...
                {fitCriteria.length ? (
                  <div className={styles.criteriaGrid}>
                    ...
                    <AverageScoreCard title="Overall behavioural score" value={behaviouralAverage} />
                  </div>
                ) : (
              ...
              <section className={styles.formSection}>
                ...
                {caseCriteria.length ? (
                  <div className={styles.criteriaGrid}>
                    ...
                    <AverageScoreCard title="Overall case score" value={caseAverage} />
                  </div>
                ) : (
