...
const RECENT_ASSIGNMENT_DAYS = 90;
const MILLISECONDS_IN_DAY = 24 * 60 * 60 * 1000;

const filterRecentAssignments = (items: InterviewerAssignmentView[]): InterviewerAssignmentView[] => {
  const cutoff = Date.now() - RECENT_ASSIGNMENT_DAYS * MILLISECONDS_IN_DAY;
  return items.filter((item) => {
    const referenceDate = item.evaluationUpdatedAt ?? item.invitationSentAt ?? null;
    if (!referenceDate) {
      return true;
    }
...
const prettifyCriterionTitle = (value: string): string => {
  ...
  return withSpaces.replace(/\b\w/g, (symbol) => symbol.toUpperCase());
};

const extendCriteriaWithForm = (
  base: CriterionDefinition[],
  formCriteria: EvaluationCriterionScore[] | undefined
): CriterionDefinition[] => {
  if (!formCriteria?.length) {
    return base;
  }
  const known = new Map(base.map((item) => [item.id, item]));
  const additions: CriterionDefinition[] = [];
  for (const entry of formCriteria) {
    const id = entry.criterionId?.trim();
    if (!id || known.has(id)) {
      continue;
    }
    const fallback: CriterionDefinition = {
      id,
      title: prettifyCriterionTitle(id),
      ratings: {}
    };
...
export const InterviewerScreen = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const filtered = filterRecentAssignments(items);
        setAssignments(filtered);
        if (filtered.length && !selectedSlot) {
          setSelectedSlot(filtered[0].slotId);
        } else if (filtered.length === 0) {
          setSelectedSlot(null);
        } else if (selectedSlot) {
          const stillExists = filtered.some((item) => item.slotId === selectedSlot);
          if (!stillExists) {
            setSelectedSlot(filtered[0]?.slotId ?? null);
          }
        }
      } catch (error) {
  ...
  const refreshAssignments = async () => {
    ...
    try {
      ...
      if (filtered.length === 0) {
        ...
        const exists = filtered.some((item) => item.slotId === selectedSlot);
        if (!exists) {
          setSelectedSlot(filtered[0].slotId);
        }
      } else {
  ...
  const renderDetail = () => {
    ...
    const baseFitCriteria: CriterionDefinition[] = fitQuestion?.criteria ?? [];
    ...
    const baseCaseCriteria = sortCaseCriteria(Array.from(mergedCaseCriteriaMap.values()));
    ...
    const fitCriteria = extendCriteriaWithForm(baseFitCriteria, displayedFormRecord?.fitCriteria);
    const caseCriteria = extendCriteriaWithForm(baseCaseCriteria, displayedFormRecord?.caseCriteria);
    ...
    const ownFitRatingsComplete = areRatingsComplete(baseFitCriteria, currentFormState.fitCriteria);
    const ownCaseRatingsComplete = areRatingsComplete(baseCaseCriteria, currentFormState.caseCriteria);
