  ...
  const value = useMemo<AppStateContextValue>(() => ({
    ...
    candidates: {
      ...
      saveProfile: async (profile, expectedVersion) => {
        ...
        const sanitized: CandidateProfile = {
          ...
          targetPractice: profile.targetPractice ?? undefined,
          targetOffice: profile.targetOffice?.trim() ?? '',
    ...
    evaluations: {
      ...
      removeEvaluation: async (id) => {
        try {
          const removedId = await evaluationsApi.remove(id);
          setEvaluations((prev) => prev.filter((item) => item.id !== removedId));
          return { ok: true, data: removedId };
        } catch (error) {
          if (error instanceof ApiError && error.code === 'not-found') {
            return { ok: false, error: 'not-found' };
          }
