...
interface AppStateContextValue {
  ...
  accounts: {
    list: AccountRecord[];
    inviteAccount: (email: string, role: AccountRole, name: string) => Promise<DomainResult<AccountRecord>>;
...
export const AppStateProvider = ({ children }: { children: ReactNode }) => {
  ...
  const value = useMemo<AppStateContextValue>(() => ({
    ...
    accounts: {
      list: accounts,
      inviteAccount: async (email, role, name) => {
        ...
        try {
          const account = await accountsApi.invite(trimmed, role, normalizedName);
