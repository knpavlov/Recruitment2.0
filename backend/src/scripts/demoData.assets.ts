...
// Подгружаем существующие критерии для fit-вопросов без создания новых записей
export const hydrateFitCriteriaCatalog = async (
  client: QueryableClient,
  fitQuestionMap: Map<string, string>,
  logInfo: (message: string) => void = () => {}
) => {
  if (fitQuestionMap.size === 0) {
    throw new Error('Не удалось подготовить каталог фит-критериев: отсутствуют fit-вопросы в базе.');
  }

  const questionIds = [...fitQuestionMap.values()];
  const criteriaResult = await client.query<{
    id: string;
    question_id: string;
    title: string;
  }>(`SELECT id, question_id, title FROM fit_question_criteria WHERE question_id = ANY($1::uuid[]);`, [questionIds]);

  const groupedByQuestion = new Map<string, Array<{ id: string; normalizedTitle: string }>>();

  for (const row of criteriaResult.rows) {
    const entries = groupedByQuestion.get(row.question_id) ?? [];
    entries.push({ id: row.id, normalizedTitle: normalizeText(row.title) });
    groupedByQuestion.set(row.question_id, entries);
  }

  for (const definition of Object.values(fitCriteriaCatalog)) {
    const questionId = fitQuestionMap.get(definition.question);
    if (!questionId) {
      throw new Error(`Не найден fit-вопрос для ключа "${definition.question}".`);
    }

    const normalizedTitle = normalizeText(definition.title);
    const candidates = groupedByQuestion.get(questionId) ?? [];
    const matched = candidates.find((candidate) => candidate.normalizedTitle === normalizedTitle);

    if (!matched) {
      throw new Error(
        `Не удалось найти критерий "${definition.title}" для fit-вопроса "${definition.question}" в базе данных.`
      );
    }

    definition.id = matched.id;
  }

  logInfo('Каталог фит-критериев считан из базы данных.');
};

// Подгружаем глобальные критерии кейсов без добавления новых строк
export const hydrateCaseCriteriaCatalog = async (
  client: QueryableClient,
  logInfo: (message: string) => void = () => {}
) => {
  const expectedTitles = Array.from(
    new Set(Object.values(caseCriteriaCatalog).map((definition) => definition.title))
  );
  ...
  const criteriaResult = await client.query<{ id: string; title: string }>(
    `SELECT id, title FROM case_criteria WHERE title = ANY($1::text[]);`,
    [expectedTitles]
  );

  const normalizedMap = new Map<string, string>();
  for (const row of criteriaResult.rows) {
    normalizedMap.set(normalizeText(row.title), row.id);
  }

  for (const definition of Object.values(caseCriteriaCatalog)) {
    const resolvedId = normalizedMap.get(normalizeText(definition.title));
    if (!resolvedId) {
      throw new Error(`Не найден глобальный критерий кейса "${definition.title}" в базе данных.`);
    }
    definition.id = resolvedId;
  }

  logInfo('Глобальные критерии кейса считаны из базы данных.');
};
