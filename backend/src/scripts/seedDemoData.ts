...
import {
  caseCriteriaCatalog,
  caseFolderReferences,
  ensureCaseLibraryAssets,
  fitCriteriaCatalog,
  fitQuestionReferences,
  fitQuestionSeeds,
  hydrateCaseCriteriaCatalog,
  hydrateFitCriteriaCatalog,
...
const CASE_FOLDER_REFERENCES: CaseFolderReference[] = caseFolderReferences;
...
const mapFitCriteriaToPayload = (
  ...
  for (const key of expectedKeys) {
    ...
    if (!definition || !definition.id) {
      throw new Error(`Не удалось сопоставить обязательный фит-критерий "${key}" с записью в базе.`);
    }
  ...
  for (const [key, value] of resolved.entries()) {
    ...
    const definition = fitCriteriaCatalog[key];
    if (!definition || !definition.id) {
      throw new Error(`Не удалось сопоставить дополнительный фит-критерий "${key}" с записью в базе.`);
    }
    payload.push({
      criterionId: definition.id,
      score: value.score,
...
const mapCaseCriteriaToPayload = (criteria: CaseCriterionSeed[] | undefined) => {
  ...
  for (const entry of criteria) {
    ...
    if (!definition || !definition.id) {
      throw new Error(`Не найден глобальный критерий кейса для ключа "${key}".`);
    }
    payload.push({
      criterionId: definition.id,
      score: entry.score,
...
const pickDeterministicItem = <T>(items: readonly T[], seed: string): T => {
  if (items.length === 0) {
    throw new Error('Невозможно выбрать элемент из пустого набора.');
  }
...
const loadCaseFolderMap = async (client: DatabaseClient, logWarn: (message: string) => void = console.warn) => {
  // Поднимаем полный список кейс-папок, чтобы можно было подобрать замену, если ожидаемое имя не найдено
  const availableFolders = await client.query<{ id: string; name: string }>(
    `SELECT id, name FROM case_folders ORDER BY updated_at DESC, id DESC;`
  );

  if (availableFolders.rows.length < CASE_FOLDER_REFERENCES.length) {
    throw new Error(
      `Not enough case folders in the database: expected ${CASE_FOLDER_REFERENCES.length}, found ${availableFolders.rows.length}.`
    );
  }

  const map = new Map<CaseFolderKey, string>();
  const usedFolderIds = new Set<string>();
  const unresolved: CaseFolderReference[] = [];

  for (const reference of CASE_FOLDER_REFERENCES) {
    const directMatch = availableFolders.rows.find((row) => row.name === reference.name);
    if (directMatch) {
      map.set(reference.key, directMatch.id);
      usedFolderIds.add(directMatch.id);
    } else {
  ...
  if (unresolved.length > 0) {
    const fallbackPool = availableFolders.rows.filter((row) => !usedFolderIds.has(row.id));
    let fallbackIndex = 0;

    for (const reference of unresolved) {
      const fallback = fallbackPool[fallbackIndex];
      if (!fallback) {
        throw new Error(
          `Unable to resolve case folder for "${reference.name}". Please ensure at least ${CASE_FOLDER_REFERENCES.length} folders exist.`
        );
      }

      fallbackIndex += 1;
      map.set(reference.key, fallback.id);
      usedFolderIds.add(fallback.id);
      logWarn(
        `Case folder "${reference.name}" not found. Using existing folder "${fallback.name}" instead.`
      );
    }
  }
...
const loadFitQuestionMap = async (client: DatabaseClient, logWarn: (message: string) => void = console.warn) => {
  // Аналогичная логика: сперва ищем точные совпадения по short_title, затем подбираем оставшиеся вопросы
  ...
  if (availableQuestions.rows.length < FIT_QUESTION_REFERENCES.length) {
    throw new Error(
      `Not enough fit questions in the database: expected ${FIT_QUESTION_REFERENCES.length}, found ${availableQuestions.rows.length}.`
    );
  }

  const map = new Map<FitQuestionKey, string>();
  const usedQuestionIds = new Set<string>();
  const unresolved: FitQuestionReference[] = [];

  for (const reference of FIT_QUESTION_REFERENCES) {
    const directMatch = availableQuestions.rows.find((row) => row.short_title === reference.shortTitle);
    if (directMatch) {
      map.set(reference.key, directMatch.id);
      usedQuestionIds.add(directMatch.id);
      const seed = fitQuestionSeeds.find((question) => question.key === reference.key);
      if (seed) {
        seed.id = directMatch.id;
      }
    } else {
      unresolved.push(reference);
    }
  ...
  if (unresolved.length > 0) {
    ...
    for (const reference of unresolved) {
      ...
      if (!fallback) {
        throw new Error(
          `Unable to resolve fit question "${reference.shortTitle}". Please ensure at least ${FIT_QUESTION_REFERENCES.length} questions exist.`
        );
      }
      ...
      logWarn(
        `Fit question "${reference.shortTitle}" not found. Using existing question "${fallback.short_title}" instead.`
      );
    }
  }
...
export const seedDemoData = async (
  ...
  try {
    ...
    logInfo('Синхронизируем кейс-библиотеку...');
    await ensureCaseLibraryAssets(client, logInfo);

    logInfo('Определяем доступные кейсы и fit-вопросы...');
    const caseFolderMap = await loadCaseFolderMap(client, logWarn);
    const fitQuestionMap = await loadFitQuestionMap(client, logWarn);

    logInfo('Подгружаем существующие фит-критерии...');
    await hydrateFitCriteriaCatalog(client, fitQuestionMap, logInfo);

    logInfo('Подгружаем глобальные критерии кейса...');
    await hydrateCaseCriteriaCatalog(client, logInfo);

    const caseFolderKeys = Array.from(caseFolderMap.keys());
    const fitQuestionKeys = Array.from(fitQuestionMap.keys());

    if (caseFolderKeys.length === 0) {
      throw new Error('В базе отсутствуют кейсы для назначения интервью.');
    }
    if (fitQuestionKeys.length === 0) {
      throw new Error('В базе отсутствуют fit-вопросы для назначения интервью.');
    }
    ...
    for (const candidate of candidates) {
      ...
      const resolvedRounds = candidate.evaluation.rounds.map((round) => {
        ...
        const assignments = round.interviews.map((interview, index) => {
          const seedBase = `${candidate.key}:${round.roundNumber}:${interview.slotId}`;
          const selectedCaseKey = pickDeterministicItem(caseFolderKeys, `${seedBase}:case`);
          const caseFolderId = caseFolderMap.get(selectedCaseKey);
          if (!caseFolderId) {
            throw new Error(`Не удалось подобрать кейс для интервью ${interview.slotId}.`);
          }

          const selectedFitKey = pickDeterministicItem(fitQuestionKeys, `${seedBase}:fit`);
          const fitQuestionId = fitQuestionMap.get(selectedFitKey);
          if (!fitQuestionId) {
            throw new Error(`Не удалось подобрать fit-вопрос для интервью ${interview.slotId}.`);
          }
          ...
          return {
            ...
            caseFolderId,
            fitQuestionId,
            fitQuestionKey: selectedFitKey,
            submissionIndex: index
          };
        });

        const interviews = assignments.map((assignment) => ({
          id: assignment.base.slotId,
          interviewerName: assignment.interviewerName,
          interviewerEmail: assignment.base.interviewerEmail,
          caseFolderId: assignment.caseFolderId,
          fitQuestionId: assignment.fitQuestionId
        }));

        const forms = assignments.map((assignment) => {
          const submissionHour =
            assignment.base.submittedHour ??
            BASE_FORM_SUBMISSION_HOUR + assignment.submissionIndex * INTERVIEW_OFFSET_HOURS;
          const submittedAt = daysAgo(
            assignment.base.submittedDaysAgo,
            submissionHour,
            assignment.submissionIndex % 2 === 0 ? 20 : 45
          ).toISOString();

          return {
            slotId: assignment.base.slotId,
            interviewerName: assignment.interviewerName,
            submitted: true,
            submittedAt,
            notes: assignment.base.notes,
            fitScore: assignment.base.fitScore,
            caseScore: assignment.base.caseScore,
            fitNotes: assignment.base.fitNotes,
            caseNotes: assignment.base.caseNotes,
            interestNotes: assignment.base.interestNotes,
            issuesToTest: assignment.base.issuesToTest,
            offerRecommendation: assignment.base.offerRecommendation,
            fitCriteria: mapFitCriteriaToPayload(
              assignment.fitQuestionKey,
              assignment.base.fitCriteria,
              assignment.base.fitScore
            ),
            caseCriteria: mapCaseCriteriaToPayload(assignment.base.caseCriteria)
          };
        });
        ...
        return {
          ...
          payload: {
            roundNumber: round.roundNumber,
            interviewCount: assignments.length,
            interviews,
            forms,
            fitQuestionId: interviews[0]?.fitQuestionId ?? null,
      ...
      const roundsPayload = resolvedRounds.map((entry) => entry.payload);

      const roundsByNumber = new Map(
        resolvedRounds.map((entry) => [entry.roundNumber, entry.payload])
      );
      ...
      for (const round of resolvedRounds) {
        totalInterviews += round.assignments.length;
        for (const assignment of round.assignments) {
          const { base, interviewerName, caseFolderId, fitQuestionId } = assignment;
          const assignmentId = toUuid(`assignment:${candidate.key}:${base.slotId}`);
          const invitationSentAt = daysAgo(base.invitationSentDaysAgo, 7, 15).toISOString();
          const checksum = computeChecksum(base.interviewerEmail, interviewerName, caseFolderId, fitQuestionId);
