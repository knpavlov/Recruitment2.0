...
import {
  ...
  ensureCaseCriteriaCatalog,
  ensureCaseLibraryAssets,
  fitCriteriaCatalog,
  fitQuestionReferences,
  fitQuestionSeeds,
  ensureFitQuestionAssets,
...
const FIT_QUESTION_SEEDS = fitQuestionSeeds;
...
const LEGACY_FIT_CRITERION_ALIASES: Record<FitQuestionKey, Record<string, FitCriterionKey>> = {
  ...
  collaboration: {
    ...
    'fit-drive': 'collaborationCommunication'
  }
};
...
const loadFitQuestionMap = async (client: DatabaseClient, logWarn: (message: string) => void = console.warn) => {
  ...
  for (const seed of FIT_QUESTION_SEEDS) {
    const byId = availableQuestions.rows.find((row) => row.id === seed.id);
    if (byId) {
      map.set(seed.key, byId.id);
      usedQuestionIds.add(byId.id);
      continue;
    }

    const byTitle = availableQuestions.rows.find(
      (row) => row.short_title === seed.shortTitle && !usedQuestionIds.has(row.id)
    );
    if (byTitle) {
      map.set(seed.key, byTitle.id);
      usedQuestionIds.add(byTitle.id);
      continue;
    }

    const reference = FIT_QUESTION_REFERENCES.find((item) => item.key === seed.key);
    if (reference) {
      unresolved.push(reference);
    }
  }
...
export const seedDemoData = async (
  ...
  try {
    ...
    logInfo('Synchronising case folders, files and criteria...');
    await ensureCaseLibraryAssets(client, logInfo);

    logInfo('Synchronising fit questions and criteria...');
    await ensureFitQuestionAssets(client, logInfo);

    logInfo('Synchronising global case criteria...');
    await ensureCaseCriteriaCatalog(client, logInfo);

    logInfo('Resolving existing case folders and fit questions...');
