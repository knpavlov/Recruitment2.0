...
// Набор ключевых слов для эвристического сопоставления критериев кейса
const CASE_CRITERION_KEYWORDS: Record<CaseCriterionKey, string[]> = {
  structure: ['structure', 'logic'],
  quant: ['quant'],
  communication: ['commun'],
  problemSolving: ['problem'],
  insight: ['insight'],
  rigor: ['rigor', 'rigour', 'detail'],
  creativity: ['creativ'],
  synthesis: ['synth'],
  clientImpact: ['impact']
};

// Набор ключевых слов для сопоставления критериев fit по названию
const FIT_CRITERION_KEYWORDS: Record<FitCriterionKey, string[]> = {
  clientCommunication: ['listen', 'commun'],
  clientOwnership: ['influenc', 'owner'],
  clientDrive: ['drive', 'resilien'],
  leadershipDirection: ['ambition', 'direction'],
  leadershipResilience: ['impact'],
  leadershipGrowth: ['drive', 'growth'],
  collaborationAlignment: ['align', 'insight'],
  collaborationCommunication: ['struct', 'commun'],
  collaborationExecution: ['execut', 'commun']
};

// Универсальная нормализация заголовков для устойчивого сравнения
...
const mapFitCriteriaToPayload = (
  ...
  resolvedFitCriteria: Map<FitCriterionKey, string>,
  ...
  for (const key of expectedKeys) {
    ...
    const criterionId = resolvedFitCriteria.get(key);

    if (!criterionId) {
      logWarn(
        `Критерий fit "${definition.title}" отсутствует в базе и будет пропущен в форме.`
      );
      continue;
    }
  ...
  for (const [key, value] of resolved.entries()) {
    ...
    if (!criterionId) {
      logWarn(
        `Не удалось сопоставить дополнительный критерий fit "${key}" с существующей записью. Значение пропущено.`
      );
      continue;
    }
...
const mapCaseCriteriaToPayload = (
  criteria: CaseCriterionSeed[] | undefined,
  resolvedCaseCriteria: Map<CaseCriterionKey, string>,
  logWarn: (message: string) => void
) => {
  ...
  for (const entry of criteria) {
    ...
    const criterionId = resolvedCaseCriteria.get(key);

    if (!criterionId) {
      logWarn(
        `Критерий кейса "${key}" не найден среди существующих записей и будет пропущен в форме.`
      );
      continue;
    }
...
const loadFitQuestionDirectory = async (
  ...
  const referenceByShortTitle = new Map(
    FIT_QUESTION_REFERENCES.map((reference) => [reference.shortTitle.trim().toLowerCase(), reference.key])
  );

  const recognized: FitQuestionRecord[] = [];
  const missingKeys = new Set(FIT_QUESTION_REFERENCES.map((reference) => reference.key));

  for (const row of availableQuestions.rows) {
    const normalizedTitle = row.short_title.trim().toLowerCase();
    const matchedKey = referenceByShortTitle.get(normalizedTitle);

    if (!matchedKey) {
      logWarn(
        `Fit question "${row.short_title}" не распознана сидером и будет пропущена при назначении.`
      );
      continue;
    }

    recognized.push({ id: row.id, shortTitle: row.short_title, key: matchedKey });
    missingKeys.delete(matchedKey);
  }

  if (missingKeys.size > 0) {
    throw new Error(
      `Unable to resolve fit questions for keys: ${[...missingKeys].join(', ')}. Ensure the base questions exist.`
    );
  }

  return {
    list: recognized,
    map: new Map(recognized.map((entry) => [entry.key, entry.id]))
  };
};
...
const hydrateCaseCriteriaCatalog = async (
  client: DatabaseClient,
  logWarn: (message: string) => void
): Promise<Map<CaseCriterionKey, string>> => {
  const result = await client.query<{ id: string; title: string }>(
    `SELECT id, title FROM case_criteria;`
  );

  if (!result.rows || result.rows.length === 0) {
    logWarn('В таблице case_criteria нет записей. Критерии кейса будут пропущены в формах.');
    return new Map();
  }
  ...
  const unused = new Set(entries.map((entry) => entry.id));
  const resolved = new Map<CaseCriterionKey, string>();

  for (const [key, definition] of Object.entries(caseCriteriaCatalog) as [
    CaseCriterionKey,
    (typeof caseCriteriaCatalog)[CaseCriterionKey]
  ][]) {
    const normalizedTitle = normalizeTitle(definition.title);
    let match = entries.find((entry) => unused.has(entry.id) && entry.normalized === normalizedTitle);

    if (!match) {
      const keywords = CASE_CRITERION_KEYWORDS[key] ?? [];
      match = entries.find(
        (entry) =>
          unused.has(entry.id) && keywords.every((keyword) => entry.normalized.includes(keyword))
      );
    }

    if (!match) {
      const fallbackId = unused.values().next().value as string | undefined;
      if (!fallbackId) {
        logWarn(
          `Критерий кейса "${definition.title}" не найден в базе и будет пропущен в формах.`
        );
        continue;
      }

      const fallback = entries.find((entry) => entry.id === fallbackId)!;
      logWarn(
        `Критерий кейса "${definition.title}" не найден по названию. Используется "${fallback.title}".`
      );
      match = fallback;
    }
...
const hydrateFitCriteriaCatalog = async (
  ...
  logWarn: (message: string) => void
): Promise<Map<FitCriterionKey, string>> => {
  ...
  if (!result.rows || result.rows.length === 0) {
    logWarn(
      'В таблице fit_question_criteria нет записей. Детальные критерии fit будут пропущены в формах.'
    );
    return new Map();
  }
  ...
  const byQuestion = new Map<
    string,
    { id: string; title: string; normalized: string }[]
  >();
  ...
  for (const row of result.rows) {
    ...
    byQuestion.get(row.question_id)!.push({ id: row.id, title: row.title, normalized: normalizedTitle });
    unusedByQuestion.get(row.question_id)!.add(row.id);
  }

  const resolved = new Map<FitCriterionKey, string>();

  for (const [key, definition] of Object.entries(fitCriteriaCatalog) as [
    FitCriterionKey,
    (typeof fitCriteriaCatalog)[FitCriterionKey]
  ][]) {
    const questionId = directory.map.get(definition.question);
    if (!questionId) {
      logWarn(
        `Не найден fit-вопрос "${definition.question}" при сопоставлении критерия "${definition.title}". Значение пропускается.`
      );
      continue;
    }

    const available = byQuestion.get(questionId) ?? [];
    const unusedSet = unusedByQuestion.get(questionId) ?? new Set<string>();

    if (available.length === 0 || unusedSet.size === 0) {
      logWarn(
        `Для fit-вопроса с ID ${questionId} отсутствуют критерии в базе. Критерий "${definition.title}" будет пропущен.`
      );
      continue;
    }
    ...
    let match = available.find(
      (entry) => unusedSet.has(entry.id) && entry.normalized === normalizedTitle
    );

    if (!match) {
      const keywords = FIT_CRITERION_KEYWORDS[key] ?? [];
      match = available.find(
        (entry) =>
          unusedSet.has(entry.id) && keywords.every((keyword) => entry.normalized.includes(keyword))
      );
    }

    if (!match) {
      const fallbackId = unusedSet.values().next().value as string | undefined;
      if (!fallbackId) {
        logWarn(
          `Критерий fit "${definition.title}" для вопроса ${definition.question} не удалось сопоставить и он будет пропущен.`
        );
        continue;
      }

      const fallback = available.find((entry) => entry.id === fallbackId)!;
      logWarn(
        `Критерий fit "${definition.title}" не найден по названию. Используется "${fallback.title}".`
      );
      match = fallback;
    }

    resolved.set(key, match.id);
    unusedSet.delete(match.id);
  }
...
export const seedDemoData = async (
  ...
  try {
    ...
    const caseCriteriaMap = await hydrateCaseCriteriaCatalog(client, logWarn);
    const fitCriteriaMap = await hydrateFitCriteriaCatalog(client, fitQuestionDirectory, logWarn);
    ...
    for (const candidate of candidates) {
      ...
      const roundsPayload = candidate.evaluation.rounds.map((round) => {
        ...
        const forms = resolvedInterviews.map((interview) => ({
          ...
          caseCriteria: mapCaseCriteriaToPayload(
            interview.caseCriteria,
            caseCriteriaMap,
            logWarn
          )
        }));
