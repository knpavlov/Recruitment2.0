...
const hydrateCaseCriteriaCatalog = async (client: DatabaseClient) => {
  ...
  for (const definition of Object.values(caseCriteriaCatalog)) {
    ...
    if (!matchedId) {
      throw new Error(`Case criterion "${definition.title}" not found in the database.`);
    }
...
export const seedDemoData = async (
  ...
  try {
    ...
    await hydrateCaseCriteriaCatalog(client);
    await hydrateFitCriteriaCatalog(client, fitQuestionDirectory);
