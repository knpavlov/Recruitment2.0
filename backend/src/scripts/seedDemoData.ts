...
type CaseFolderRecord = {
  id: string;
  name: string;
};

type CaseFolderDirectory = {
  list: CaseFolderRecord[];
  map: Map<CaseFolderKey, CaseFolderRecord>;
};
...
interface FitQuestionDirectory {
  ...
  map: Map<FitQuestionKey, FitQuestionRecord>;
}
...
const CASE_FOLDER_REFERENCES = caseFolderReferences;

const loadCaseFolderDirectory = async (
  client: DatabaseClient,
  logWarn: (message: string) => void = console.warn
): Promise<CaseFolderDirectory> => {
  ...
  const normalizedByName = new Map(
    list.map((folder) => [normalizeLabel(folder.name), folder])
  );
  const used = new Set<string>();
  const assigned = new Map<CaseFolderKey, CaseFolderRecord>();

  for (const reference of CASE_FOLDER_REFERENCES) {
    const normalizedName = normalizeLabel(reference.name);
    const matched = normalizedByName.get(normalizedName);

    if (matched) {
      assigned.set(reference.key as CaseFolderKey, matched);
      used.add(matched.id);
      continue;
    }

    const fallback = list.find((folder) => !used.has(folder.id)) ?? list[0];

    if (fallback) {
      logWarn(
        `Папка кейса "${reference.name}" не найдена по названию. Используем "${fallback.name}".`
      );
      assigned.set(reference.key as CaseFolderKey, fallback);
...
const loadFitQuestionDirectory = async (
  ...
  return {
    list: ordered,
    map: new Map(ordered.map((entry) => [entry.key, entry]))
  };
};

const resolveCaseFolder = (
  directory: CaseFolderDirectory,
  key: CaseFolderKey,
  seed: string,
  logWarn: (message: string) => void,
  warned: Set<CaseFolderKey>
): CaseFolderRecord => {
  const resolved = directory.map.get(key);
  if (resolved) {
    return resolved;
  }
  ...
  if (!warned.has(key)) {
    logWarn(
      `Для ключа кейса "${key}" не найдено точного совпадения. Будет выбран случайный кейс из доступных.`
    );
    warned.add(key);
  }

  return pickDeterministicItem(`${seed}:case-fallback`, directory.list);
};

const resolveFitQuestion = (
  directory: FitQuestionDirectory,
  key: FitQuestionKey,
  seed: string,
  logWarn: (message: string) => void,
  warned: Set<FitQuestionKey>
): FitQuestionRecord => {
  const resolved = directory.map.get(key);
  if (resolved) {
    return resolved;
  ...
  if (!warned.has(key)) {
    logWarn(
      `Для ключа fit-вопроса "${key}" не найдено точного совпадения. Используем случайный вопрос из доступных.`
    );
    warned.add(key);
  }

  return pickDeterministicItem(`${seed}:fit-fallback`, directory.list);
};
...
const hydrateFitCriteriaCatalog = async (
  ...
  for (const definition of Object.values(fitCriteriaCatalog)) {
    const questionRecord = directory.map.get(definition.question);
...
export const seedDemoData = async (
  ...
  try {
    ...
    const caseFolderDirectory = await loadCaseFolderDirectory(client, logWarn);
    const fitQuestionDirectory = await loadFitQuestionDirectory(client, logWarn);

    const missingCaseKeys = new Set<CaseFolderKey>();
    const missingFitKeys = new Set<FitQuestionKey>();
    ...
    for (const candidate of candidates) {
      ...
      const roundsPayload = candidate.evaluation.rounds.map((round) => {
        ...
        const resolvedInterviews = round.interviews.map((interview, index) => {
          ...
          const assignedCaseFolder = resolveCaseFolder(
            caseFolderDirectory,
            interview.caseFolder,
            assignmentSeed,
            logWarn,
            missingCaseKeys
          );
          const assignedFitQuestion = resolveFitQuestion(
            fitQuestionDirectory,
