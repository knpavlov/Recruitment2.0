...
const CASE_CRITERION_HINTS: Record<CaseCriterionKey, string[]> = {
  structure: ['structure', 'logic'],
  quant: ['quant'],
  communication: ['communicat'],
  problemSolving: ['problem', 'solv'],
  insight: ['insight'],
  rigor: ['rigor', 'rigour', 'detail'],
  creativity: ['creativ'],
  synthesis: ['synth', 'recommend'],
  clientImpact: ['client', 'impact']
};

const FIT_CRITERION_HINTS: Record<FitCriterionKey, string[]> = {
  clientCommunication: ['communicat', 'listen'],
  clientOwnership: ['influ', 'owner', 'trust'],
  clientDrive: ['drive', 'resilien', 'grit'],
  leadershipDirection: ['ambit', 'vision'],
  leadershipResilience: ['impact', 'results'],
  leadershipGrowth: ['drive', 'energy', 'growth'],
  collaborationAlignment: ['align', 'collab'],
  collaborationCommunication: ['communicat', 'partner'],
  collaborationExecution: ['execute', 'delivery', 'outcome']
};

const normaliseText = (value: string | null | undefined) => value?.trim().toLowerCase() ?? '';
...
const loadFitQuestionDirectory = async (
  ...
  const unmatchedRows: { id: string; short_title: string }[] = [];
  ...
  for (const row of availableQuestions.rows) {
    ...
    if (matchedKey) {
      const record: FitQuestionRecord = {
        id: row.id,
        shortTitle: row.short_title,
        key: matchedKey
      };
      assigned.set(matchedKey, record);
      usedIds.add(row.id);
    } else {
  ...
  const missingKeys = FIT_QUESTION_REFERENCES.map((reference) => reference.key).filter(
    (key) => !assigned.has(key)
  );

  const fallbackPool = availableQuestions.rows.filter((row) => !usedIds.has(row.id));

  for (const missingKey of missingKeys) {
    const fallbackRow = unmatchedRows.shift() ?? fallbackPool.shift() ?? availableQuestions.rows.find((row) => !usedIds.has(row.id));

    if (!fallbackRow) {
      throw new Error(
        `Unable to resolve fit question for key "${missingKey}" — not enough records in the database.`
      );
    }

    logWarn(
      `Fit question для ключа ${missingKey} не найден по названию. Будет использована запись "${fallbackRow.short_title}".`
    );
    ...
    assigned.set(missingKey, record);
    usedIds.add(fallbackRow.id);
  }

  const recognized = FIT_QUESTION_REFERENCES.map((reference) => {
    const record = assigned.get(reference.key);
    if (!record) {
      throw new Error(`Internal error: fit question for key ${reference.key} не был сопоставлен.`);
    }
    return record;
  });
...
const hydrateCaseCriteriaCatalog = async (
  ...
  if (!result.rows || result.rows.length === 0) {
    throw new Error('Case criteria catalog is empty. Add criteria before running the demo seed.');
  }

  const rows = result.rows.map((row) => ({
    id: row.id,
    title: row.title,
    normalized: normaliseText(row.title)
  }));

  const byTitle = new Map(rows.map((row) => [row.normalized, row]));
  const usedIds = new Set<string>();
  const unresolved: Array<{ key: CaseCriterionKey; definition: (typeof caseCriteriaCatalog)[CaseCriterionKey] }> = [];

  for (const [key, definition] of Object.entries(caseCriteriaCatalog) as Array<[
    CaseCriterionKey,
    (typeof caseCriteriaCatalog)[CaseCriterionKey]
  ]>) {
    const normalizedTitle = normaliseText(definition.title);
    const directMatch = byTitle.get(normalizedTitle);

    if (directMatch && !usedIds.has(directMatch.id)) {
      definition.id = directMatch.id;
      usedIds.add(directMatch.id);
    } else {
      if (directMatch && usedIds.has(directMatch.id)) {
        logWarn(
          `Критерий кейса "${definition.title}" найден, но уже использован другой записью. Будет выполнен поиск по эвристике.`
        );
      }
      unresolved.push({ key, definition });
    }
  }

  const findBestMatch = (hints: string[]) => {
    let bestRow: { id: string; title: string; normalized: string } | undefined;
    let bestScore = 0;
    ...
    for (const row of rows) {
      ...
      const score = hints.reduce(
        (accumulator, hint) => (row.normalized.includes(hint) ? accumulator + 1 : accumulator),
        0
      );
      if (score > bestScore) {
        bestScore = score;
  ...
  for (const entry of unresolved) {
    const hints = CASE_CRITERION_HINTS[entry.key] ?? [];
    const { match, score } = findBestMatch(hints);

    if (match && score > 0) {
      entry.definition.id = match.id;
      usedIds.add(match.id);
      continue;
    }

    const fallback = rows.find((row) => !usedIds.has(row.id));
    if (!fallback) {
      throw new Error(
        `Не удалось сопоставить критерий кейса "${entry.definition.title}" — не осталось свободных записей в базе.`
      );
    }

    logWarn(
      `Критерий кейса "${entry.definition.title}" не найден по названию. Использована запись "${fallback.title}".`
    );

    entry.definition.id = fallback.id;
...
const hydrateFitCriteriaCatalog = async (
  ...
  const grouped = new Map<
    string,
    {
      rows: Array<{ id: string; title: string; normalized: string }>;
      used: Set<string>;
    }
  >();
  ...
  for (const row of result.rows) {
    if (!grouped.has(row.question_id)) {
      grouped.set(row.question_id, { rows: [], used: new Set() });
    }
    grouped.get(row.question_id)!.rows.push({
      id: row.id,
      title: row.title,
      normalized: normaliseText(row.title)
    });
  ...
  for (const [key, definition] of Object.entries(fitCriteriaCatalog) as Array<[
    FitCriterionKey,
    ...
    if (!questionId) {
      throw new Error(
        `Fit question key "${definition.question}" не найден в базе при сопоставлении критериев.`
      );
    }
    ...
    if (!group || group.rows.length === 0) {
      throw new Error(
        `Для fit вопроса с идентификатором ${questionId} отсутствуют критерии в базе.`
      );
    }

    const directMatch = group.rows.find(
      (row) => row.normalized === normaliseText(definition.title) && !group.used.has(row.id)
    );

    if (directMatch) {
      definition.id = directMatch.id;
      group.used.add(directMatch.id);
      continue;
    }

    const hints = FIT_CRITERION_HINTS[key] ?? [];
    let bestRow: { id: string; title: string; normalized: string } | undefined;
    let bestScore = 0;

    for (const row of group.rows) {
      if (group.used.has(row.id)) {
        continue;
      }
      const score = hints.reduce(
        (accumulator, hint) => (row.normalized.includes(hint) ? accumulator + 1 : accumulator),
    ...
    if (bestRow && bestScore > 0) {
      definition.id = bestRow.id;
      group.used.add(bestRow.id);
      continue;
    }

    const fallback = group.rows.find((row) => !group.used.has(row.id));
    if (!fallback) {
      throw new Error(
        `Не удалось сопоставить критерий "${definition.title}" для вопроса ${definition.question} — не осталось свободных записей.`
      );
    }

    logWarn(
      `Критерий fit "${definition.title}" не найден по названию. Использована запись "${fallback.title}".`
    );

    definition.id = fallback.id;
    group.used.add(fallback.id);
  }
};
...
export const seedDemoData = async (
  ...
  try {
    ...
    logInfo('Проверяем наличие папок кейсов и вложенных файлов...');
    await ensureCaseLibraryAssets(client, logInfo);

    logInfo('Загружаем папки кейсов и fit-вопросы из базы...');
    const caseFolders = await loadCaseFolders(client);
    const fitQuestionDirectory = await loadFitQuestionDirectory(client, logWarn);

    logInfo('Сопоставляем критерии кейсов и fit-вопросов с существующими записями...');
