...
const hydrateCaseCriteriaCatalog = async (
  ...
  if (!result.rows || result.rows.length === 0) {
    throw new Error('No case criteria found in the database.');
  }

  const byTitle = new Map(
    result.rows.map((row) => [row.title.trim().toLowerCase(), row.id])
  );
  ...
  for (const definition of Object.values(caseCriteriaCatalog)) {
    ...
    if (!fallbackId) {
      throw new Error(
        `Case criterion "${definition.title}" could not be matched to an existing record.`
      );
    }

    logWarn(
      `Категория кейса "${definition.title}" не найдена по названию. Использован доступный критерий с ID ${fallbackId}.`
    );
...
const hydrateFitCriteriaCatalog = async (
  ...
  for (const definition of Object.values(fitCriteriaCatalog)) {
    ...
    const fallbackId = fallbackSet ? fallbackSet.values().next().value : undefined;

    if (!fallbackId) {
      throw new Error(
        `Fit criterion "${definition.title}" for question "${definition.question}" not found in the database.`
      );
    }

    logWarn(
      `Критерий fit "${definition.title}" не найден по названию. Использован доступный критерий с ID ${fallbackId}.`
    );
    definition.id = fallbackId;
