...
const handleError = (error: unknown, res: Response) => {
  ...
  switch (error.message) {
    case 'INVALID_INPUT':
      ...
      return;
    case 'NOT_FOUND':
      res.status(404).json({ code: 'not-found', message: 'Criterion not found.' });
      return;
    case 'VERSION_CONFLICT':
      res.status(409).json({ code: 'version-conflict', message: 'The criterion version is outdated.' });
      return;
    default:
...
router.get('/:id', async (req, res) => {
  try {
    const criterion = await caseCriteriaService.getCriterion(req.params.id);
    res.json(criterion);
  } catch (error) {
