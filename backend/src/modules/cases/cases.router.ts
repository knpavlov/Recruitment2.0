...
const handleError = (error: unknown, res: Response) => {
  ...
  switch (error.message) {
    case 'INVALID_NAME':
    case 'INVALID_INPUT':
      res.status(400).json({ code: 'invalid-input', message: 'Invalid data.' });
      return;
    case 'DUPLICATE_NAME':
      res.status(409).json({ code: 'duplicate', message: 'A folder with this name already exists.' });
      return;
    case 'VERSION_CONFLICT':
      res.status(409).json({ code: 'version-conflict', message: 'Folder version is outdated.' });
      return;
    case 'NOT_FOUND':
...
router.post('/:id/files', async (req, res) => {
  ...
  if (!Array.isArray(files) || typeof expectedVersion !== 'number') {
    res.status(400).json({ code: 'invalid-input', message: 'Invalid request payload.' });
    return;
  }
  try {
