...
const interviewerRoles: InterviewerSeniority[] = ['MD', 'SD', 'D', 'SM', 'M', 'SA', 'A'];
...
const normalizeRoles = (value: unknown): InterviewerSeniority[] | undefined => {
  if (!value) {
    return undefined;
  }
  const toRole = (input: string) => {
    const normalized = input.trim().toUpperCase() as InterviewerSeniority;
    return interviewerRoles.includes(normalized) ? normalized : null;
  };

  if (Array.isArray(value)) {
    const result = value
      .map((item) => (typeof item === 'string' ? toRole(item) : null))
      .filter((item): item is InterviewerSeniority => Boolean(item));
...
router.get('/interviewers', async (req, res) => {
  try {
    ...
    const stats = await analyticsService.getInterviewerStats(period, { interviewerIds, roles });
    res.json(stats);
  } catch (error) {
...
router.get('/export/:dataset', async (req, res) => {
  ...
  try {
    switch (dataset) {
      ...
      case 'interviewers': {
        const period = resolveInterviewerPeriod(req.query.period);
        const csv = await analyticsService.exportInterviewers(period, { interviewerIds, roles });
        res.setHeader('Content-Type', 'text/csv; charset=utf-8');
