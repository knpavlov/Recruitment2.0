...
router.get('/summary', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Unable to fetch summary metrics.' });
  }
});
...
router.get('/timeline', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Unable to fetch the time series.' });
  }
});
...
router.get('/interviewers', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Unable to fetch interviewer statistics.' });
  }
});
...
router.get('/export/:dataset', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Unable to prepare the file.' });
  }
});
