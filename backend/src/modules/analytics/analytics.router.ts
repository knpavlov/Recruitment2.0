...
router.get('/summary', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Не удалось получить сводные метрики.' });
  }
});
...
router.get('/timeline', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Не удалось получить временной ряд.' });
  }
});
...
router.get('/interviewers', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 'analytics-error', message: 'Не удалось получить статистику интервьюеров.' });
  }
});
...
router.get('/export/:dataset', async (req, res) => {
  ...
  try {
    switch (dataset) {
      ...
      default:
        res.status(404).json({ code: 'not-found', message: 'Неизвестный тип экспорта.' });
    }
  } catch (error) {
    console.error('Failed to export analytics dataset:', error);
    res.status(500).json({ code: 'analytics-error', message: 'Не удалось подготовить файл.' });
  }
});
