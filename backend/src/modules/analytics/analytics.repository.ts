...
export class AnalyticsRepository {
  async listAccountRoles(): Promise<Array<{ email: string; interviewerRole: InterviewerSeniority | null }>> {
    const result = await postgresPool.query<{ email: string; interviewer_role: string | null }>(
      `SELECT email, interviewer_role FROM accounts`
    );
    ...
    return result.rows.map((row) => {
      ...
      const role = typeof row.interviewer_role === 'string' ? row.interviewer_role.trim().toUpperCase() : '';
      const interviewerRole = allowed.includes(role as InterviewerSeniority) ? (role as InterviewerSeniority) : null;
      return { email, interviewerRole };
    });
  }
  ...
  async listAssignments(): Promise<AssignmentSnapshotRow[]> {
    const result = await postgresPool.query<{
      ...
      `SELECT evaluation_id, slot_id, interviewer_email, interviewer_name, round_number, created_at, invitation_sent_at
