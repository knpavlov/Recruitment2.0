...
const FISCAL_YEAR_START_MONTH = 4; // Апрель
...
const startOfWeekUtc = (value: Date) => {
  ...
  const offset = (day + 6) % 7; // Смещение до понедельника
  if (offset !== 0) {
...
export class AnalyticsService {
  ...
  async getTimeline(
    ...
    for (const evaluation of evaluations) {
      ...
      for (const round of evaluation.roundHistory) {
        ...
        if (!roundDate || !isWithinRange(roundDate, alignedStart, rangeEnd)) {
          // Даже если сам раунд вне диапазона, формы могли быть отправлены позже.
        }

        if (roundDate && isWithinRange(roundDate, alignedStart, rangeEnd)) {
          const bucket = ensureBucket(roundDate);
          const interviewCount = round.interviewCount ?? round.forms.length;
          bucket.totalInterviews += interviewCount;
          if (round.roundNumber === 1) {
            bucket.firstRoundInterviews += interviewCount;
          }
          if (round.roundNumber === 2) {
            bucket.secondRoundInterviews += interviewCount;
          }
        }
  ...
  async getInterviewerStats(
    ...
    const referenceEnd = options.to ? parseDate(options.to) ?? new Date() : new Date();
    const rangeEnd = endOfDayUtc(referenceEnd);
    const defaultStart = startOfMonthUtc(addMonthsUtc(rangeEnd, -5));
    const rangeStart = options.from ? parseDate(options.from) ?? defaultStart : defaultStart;
    const alignedStart = startOfDayUtc(rangeStart);
    ...
    const relevantBuckets = Array.from(buckets.values())
      ...
      .sort((a, b) => {
        ...
        return a.interviewerName.localeCompare(b.interviewerName, 'ru');
      })
  ...
  async exportInterviewers(
    ...
    const stats = await this.getInterviewerStats(groupBy, options);
    const header = [
      'interviewer_id',
      'interviewer_name',
      'interviewer_email',
