...
export class AnalyticsService {
  ...
  async getInterviewerStats(
    ...
    const reference = new Date();
    const defaultRange = buildInterviewerRange(period, reference);
    const groupBy = options.groupBy ?? 'month';
    const overrideStart = options.from ? parseDate(options.from) : null;
    const overrideEnd = options.to ? parseDate(options.to) : null;
    let rangeStart = overrideStart ? startOfDayUtc(overrideStart) : startOfDayUtc(defaultRange.start);
    let rangeEnd = overrideEnd ? endOfDayUtc(overrideEnd) : endOfDayUtc(defaultRange.end);

    if (rangeStart.getTime() > rangeEnd.getTime()) {
      rangeStart = startOfDayUtc(rangeEnd);
    }
    ...
    for (const assignment of assignments) {
      ...
      const existingDescriptor = interviewerDescriptors.get(descriptor.id);
      if (existingDescriptor) {
        interviewerDescriptors.set(descriptor.id, {
          ...existingDescriptor,
          ...descriptor,
      ...
      if (!preferredDate || !isWithinRange(preferredDate, rangeStart, rangeEnd)) {
        continue;
      }
