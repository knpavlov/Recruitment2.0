...
export class AnalyticsService {
  ...
  async getInterviewerStats(
    ...
    const filterSet = options.interviewerIds && options.interviewerIds.length
      ? new Set(options.interviewerIds.map((id) => normalizeId(id)).filter((id) => id.length > 0))
      : null;
    ...
    for (const assignment of assignments) {
      ...
      const normalizedDescriptorId = normalizeId(descriptor.id);

      if (filterSet && !filterSet.has(normalizedDescriptorId)) {
        continue;
      }
    ...
    const relevantBuckets = Array.from(buckets.values())
      .filter((bucket) => !filterSet || filterSet.has(normalizeId(bucket.interviewerId)))
