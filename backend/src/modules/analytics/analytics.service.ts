...
const normalizeInterviewerId = (value: string): string => {
  // Приводим идентификатор к единому регистру, чтобы фильтрация работала корректно
...
export class AnalyticsService {
  ...
  async getInterviewerStats(
    ...
    const filterSet = options.interviewerIds && options.interviewerIds.length
      ? new Set(
          options.interviewerIds
            .map((id) => normalizeInterviewerId(id))
    ...
    for (const assignment of assignments) {
      ...
      if (filterSet && !filterSet.has(normalizeInterviewerId(descriptor.id))) {
        continue;
      }
    ...
    const relevantBuckets = Array.from(buckets.values())
      .filter((bucket) => !filterSet || filterSet.has(normalizeInterviewerId(bucket.interviewerId)))
