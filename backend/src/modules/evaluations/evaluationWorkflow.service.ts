...
const resolvePortalBaseUrl = (preferredUrl?: string): string => {
  // Используем несколько источников, чтобы админский интерфейс мог пробросить актуальный URL,
  // а окружение — задать продакшен-конфигурацию.
  const sources = [
    preferredUrl,
    process.env.INTERVIEW_PORTAL_URL,
    process.env.PUBLIC_INTERVIEW_PORTAL_URL,
    process.env.APP_URL,
    process.env.PUBLIC_APP_URL,
    process.env.PUBLIC_URL
  ];
  const allowLocal =
    process.env.ALLOW_LOCAL_INTERVIEW_PORTAL === 'true' || process.env.NODE_ENV !== 'production';
  ...
  for (const source of sources) {
    ...
    if (localHost) {
      console.warn('[Evaluations] Используется локальный URL интервьюерского портала:', normalized);
    }
    return normalized;
  }
...
export class EvaluationWorkflowService {
  ...
  async startProcess(id: string, options?: { portalUrl?: string }) {
    ...
    const portalBaseUrl = resolvePortalBaseUrl(options?.portalUrl);
