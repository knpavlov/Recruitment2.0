...
export class EvaluationWorkflowService {
  ...
  async updateDecision(
    ...
    const writeModel = buildWriteModelFromRecord(evaluation);
    const normalizedDecision = decision === 'accepted-offer' ? 'offer' : decision;
    ...
    if (normalizedDecision === 'offer') {
      ...
      let resolvedStatus: OfferDecisionStatus = allowedStatuses.includes(currentStatus ?? 'pending')
        ? (currentStatus as OfferDecisionStatus)
        : 'pending';
      if (decision === 'accepted-offer' && (currentStatus === null || currentStatus === undefined || currentStatus === 'pending')) {
        resolvedStatus = 'accepted';
      }
      writeModel.offerDecisionStatus = resolvedStatus;
    } else {
      writeModel.offerDecisionStatus = 'pending';
    }
