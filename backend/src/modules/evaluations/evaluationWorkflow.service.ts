...
export class EvaluationWorkflowService {
  ...
  private async loadEvaluationWithState(id: string): Promise<EvaluationRecord> {
    ...
    const assignments = await this.evaluations.listAssignmentsForEvaluation(id, record.roundNumber ?? 1);
  ...
  async listAssignmentsForInterviewer(email: string): Promise<InterviewerAssignmentView[]> {
    ...
    return assignments.map((assignment) => {
      ...
      if (!form && evaluation) {
        const snapshot = evaluation.roundHistory.find((entry) => entry.roundNumber === assignment.roundNumber);
        if (snapshot) {
          form = snapshot.forms.find((item) => item.slotId === assignment.slotId) ?? null;
        }
      }
