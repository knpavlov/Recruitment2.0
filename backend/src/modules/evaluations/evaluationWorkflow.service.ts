...
const readCriteriaPayload = (
  value: unknown
): EvaluationRecord['forms'][number]['fitCriteria'] | undefined => {
  if (!Array.isArray(value)) {
    return undefined;
  }
  const criteria: EvaluationRecord['forms'][number]['fitCriteria'] = [];
  for (const entry of value) {
    ...
    criteria.push({ criterionId, score, notes });
  }
  return criteria;
};

const averageScore = (items: Array<{ score?: number }>): number | undefined => {
  const scores = items
...
export class EvaluationWorkflowService {
  ...
  async submitInterviewForm(
    ...
    const fitCriteria = readCriteriaPayload(payload.fitCriteria) ?? currentForm.fitCriteria ?? [];
    const caseCriteria = readCriteriaPayload(payload.caseCriteria) ?? currentForm.caseCriteria ?? [];

    const fitScoreValue = readScore(payload.fitScore) ?? averageScore(fitCriteria) ?? currentForm.fitScore;
    const caseScoreValue = readScore(payload.caseScore) ?? averageScore(caseCriteria) ?? currentForm.caseScore;
    ...
    const updatedForms = evaluation.forms.map((form) => {
      ...
      return {
        ...
        fitCriteria,
        caseCriteria,
        interestLevel: typeof payload.interestLevel === 'string' ? payload.interestLevel : form.interestLevel,
        ...
        followUpPlan: typeof payload.followUpPlan === 'string' ? payload.followUpPlan : form.followUpPlan
      };
    });
