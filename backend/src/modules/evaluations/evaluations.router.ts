...
router.post('/:id/start', async (req, res) => {
  ...
  const override = typeof portalUrl === 'string' && portalUrl.trim() ? portalUrl : undefined;

  try {
    const result = await evaluationWorkflowService.startProcess(req.params.id, { portalUrl: override });
    res.json(result);
  } catch (error) {
