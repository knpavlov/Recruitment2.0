  ...
  const caseScoreValue =
    typeof caseScore === 'number'
      ...
      : typeof caseScore === 'string'
        ? caseScore
        : undefined;
  const fitCriteriaValue =
    fitCriteria && typeof fitCriteria === 'object' ? (fitCriteria as Record<string, unknown>) : undefined;
  const caseCriteriaValue =
    caseCriteria && typeof caseCriteria === 'object' ? (caseCriteria as Record<string, unknown>) : undefined;
  try {
    const result = await evaluationWorkflowService.submitInterviewForm(
      ...
      email,
      {
        ...
        fitCriteria: fitCriteriaValue,
        caseCriteria: caseCriteriaValue,
        interestLevel: typeof interestLevel === 'string' ? interestLevel : undefined,
        issuesToTest: typeof issuesToTest === 'string' ? issuesToTest : undefined,
        summary: typeof summary === 'string' ? summary : undefined,
        offerRecommendation:
          typeof offerRecommendation === 'string' ? offerRecommendation : undefined,
        offerRecommendationNotes:
          typeof offerRecommendationNotes === 'string' ? offerRecommendationNotes : undefined
      }
    );
    res.json(result);
  } catch (error) {
