...
const mapCriteriaList = (value: unknown): InterviewStatusModel['fitCriteria'] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const list: InterviewStatusModel['fitCriteria'] = [];
  for (const entry of value) {
    if (!entry || typeof entry !== 'object') {
      continue;
    }
...
const mapForms = (value: unknown): InterviewStatusModel[] => {
  ...
  for (const entry of value) {
    ...
    const overallImpression =
      item.overallImpression === 'top-choice' ||
      item.overallImpression === 'strong' ||
      item.overallImpression === 'mixed' ||
      item.overallImpression === 'concerns'
        ? item.overallImpression
        : undefined;
    const offerRecommendation =
      item.offerRecommendation === 'yes-priority' ||
      item.offerRecommendation === 'yes' ||
      item.offerRecommendation === 'hold' ||
      item.offerRecommendation === 'no'
        ? item.offerRecommendation
        : undefined;
    const followUpPlan = typeof item.followUpPlan === 'string' ? item.followUpPlan : undefined;
