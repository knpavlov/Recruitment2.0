...
router.post('/request-code', async (req, res) => {
  try {
    ...
    res.status(404).json({ message: 'Account not found or access is denied.' });
  }
});
...
router.post('/verify-code', async (req, res) => {
  ...
  if (!email || !code) {
    res.status(400).json({ message: 'Provide both email and access code.' });
    return;
  }
  try {
    ...
    if (error instanceof Error && error.message === 'CODE_EXPIRED') {
      res.status(410).json({ message: 'The code has expired. Request a new one.' });
      return;
    }
